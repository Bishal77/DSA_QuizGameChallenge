...
def get_info():  # to get the information of the player
    ...
    n = Label(info, text="Name", width=15, anchor=W).grid(
        row=0, column=0, columnspan=1)
    r = Label(info, text="Registration No.", width=15,
              anchor=W).grid(row=1, column=0)
    name = Entry(info)
    regno = Entry(info)
    name.grid(row=0, column=1, pady=10)
    regno.grid(row=1, column=1, pady=20)
    back_button = Button(info, text="Back", width=10, height=2,  activebackground="red",
                         command=lambda: back(info)).grid(row=2, column=0, sticky="nsew")
    s = Button(info, text="SUBMIT", width=10, height=2,  activebackground="blue", command=lambda: submit(
        name.get(), regno.get(), info)).grid(row=2, column=1, sticky="nsew")
...
def start_game():  # to start the game
    ...
    game_handle.launch_game(question_stacks, 1, 1,
                            player, 10)  # starting the game
    hs = update_highscore()
    resultFrame = LabelFrame(root, text="Result", padx=250, pady=250)
    result = Label(resultFrame, text=player.name + ", your score is: " +
                   str(player.score), fg="green", font=80, pady=20)
    HS = Label(resultFrame, text="High Score: " +
               hs, fg="blue", font=80, pady=20)
    resultFrame.place(x=700, y=250)
    result.pack()  # displaying the result
    HS.pack()
    back_button = Button(resultFrame, text="Back", width=10, height=2,
                         activebackground="blue", command=lambda: back(resultFrame)).pack()
...
def get_passcode():  # to get the passcode to add the question
    ...
    back_button = Button(info, text="Back", width=10, height=2,  activebackground="red",
                         command=lambda: back(info)).grid(row=1, column=0, sticky="nsew")
    s = Button(info, text="SUBMIT", width=10, height=2, activebackground="blue",
               command=lambda: check_passcode(passcode.get(), info)).grid(row=1, column=1, sticky="nsew")
...
def create_homepage():  # to create the homepage
    ...
    start_G = Button(frame, text="Start Game", width=20, height=3,
                     activebackground="blue", command=get_info).grid(pady=10)
    add_Q = Button(frame, text="Add Questions", width=20,
                   activebackground="blue", command=get_passcode).grid(pady=10)
    button_exit = Button(frame, text='Exit', width=20,
                         activebackground="red", command=root.quit).grid()
...
def easy(ADQ):  # to add easy quesetion
    ...
    f.write("**||**\n")
    f.close()
    label1 = Label(ADQ, text="You added an easy question!",
                   fg="blue", font="100")
    back_button = Button(ADQ, text="Done", width=10, height=2,
                         activebackground="blue", command=lambda: back(ADQ))
    add_button = Button(ADQ, text="Add Again", width=10,  height=2,
                        activebackground="blue", command=lambda: add_again(ADQ))
...
def medium(ADQ):  # to add medium question
    ...
    f.write("**||**\n")
    f.close()
    label1 = Label(ADQ, text="You added a medium question!",
                   fg="blue", font="100")
    back_button = Button(ADQ, text="Back", width=10, height=2,
                         activebackground="blue", command=lambda: back(ADQ))
    add_button = Button(ADQ, text="Add Again", width=10,  height=2,
                        activebackground="blue", command=lambda: add_again(ADQ))
    label1.grid(row=0, column=0, columnspan=2, pady=10)
...
def hard(ADQ):  # to add hard question
    ...
    f.write("**||**\n")
    f.close()
    label1 = Label(ADQ, text="You added a hard question!",
                   fg="blue", font="100")
    back_button = Button(ADQ, text="Back", width=10, height=2,
                         activebackground="blue", command=lambda: back(ADQ))
    add_button = Button(ADQ, text="Add Again", width=10,  height=2,
...
def hot(ADQ):  # to add HOT question
    ...
    f.write("**||**\n")
    f.close()
    label1 = Label(ADQ, text="You added a HOT question!",
                   fg="blue", font="100")
    back_button = Button(ADQ, text="Back", width=10, height=2,
                         activebackground="blue", command=lambda: back(ADQ))
    add_button = Button(ADQ, text="Add Again", width=10,  height=2,
...
def add_questions():  # to prompt what type of question is to be added
    ...
    label1 = Label(ADQ, text="Choose difficulty level:",
                   font="100").grid(row=0, column=0, columnspan=4)
    Easy = Button(ADQ, text="Easy", width=10,
                  height=2, command=lambda: easy(ADQ))
    Medium = Button(ADQ, text="Medium", width=10,
                    height=2, command=lambda: medium(ADQ))
    Hard = Button(ADQ, text="Hard", width=10,
                  height=2, command=lambda: hard(ADQ))
